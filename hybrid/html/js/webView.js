  ...
  var e = {},
    ...
    i = function (e, i) {
      ...
      if (s) {
        ...
        plus.webview
          .getLaunchWebview()
          .evalJS('UniPlusBridge.subscribeHandler("WEB_INVOKE_APPSERVICE",' + r + ',' + JSON.stringify(o) + ');')
      } else
    ...
    c
      ? ((m = e),
        window.plus && /complete|loaded|interactive/.test(document.readyState)
          ? p()
          : document.addEventListener('plusready', p))
      : g
      ? ((m = n),
        window.WeixinJSBridge && WeixinJSBridge.invoke ? p() : document.addEventListener('WeixinJSBridgeReady', p))
      : ((m = w ? t : u ? a : v ? r : e), document.addEventListener('DOMContentLoaded', p)),
    m
  )
})
